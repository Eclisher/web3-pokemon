...
const PokemonList = ({ pokemonList, basePath }) => {
  ...
  const handleDetailsClick = (id) => {
    const basePathWithId = router.pathname.includes('/server') ? '/server' : '/client';
      ...
      <ul className={styles.pokemonList}>
        {pokemonList.map((pokemon, index) => (
          <li key={index} className={styles.pokemonListItem}>
            ...
            <button className={styles.pokemonButton} onClick={() => handleDetailsClick(index + 1)}>Détails</button>
          </li>
        ))}
      </ul>
    </div>
